    ...
    >
      ...
      <ol className="space-y-3 mb-8 list-none">
        <li className="flex items-center gap-3">
          <StepIcon n={1} />
          <span className="text-lg text-sjsublue">Enter your name</span>
        </li>
        <br />
        <li className="flex items-center gap-3">
          <StepIcon n={2} />
          <span className="text-lg text-sjsublue">
            Press <span className="font-semibold">Send the email</span>
          </span>
        </li>
      </ol>
      ...
      <form className="space-y-5" onSubmit={onSubmit}>
        <div>
          <label htmlFor={inputId} className="block text-lg font-semibold text-sjsublue">
            Your name
          </label>
          <input
            id={inputId}
        ...
        <p id={helpId} className="text-sm text-clipper">
          This opens your email app with a pre‑filled message to <span className="font-medium">{to.join(", ")}</span>
          {bcc?.length ? (
            <>
              {" "}with <span className="font-medium">{bcc.join(", ")}</span> BCC’d.
            </>
          ) : (
            "."
          )}
